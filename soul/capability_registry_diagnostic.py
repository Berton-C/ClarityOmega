...
def parse_tests(output):
    """
    Extract the test results from this PeTTa evaluator's output.

    IMPORTANT (learned from real runs): run.sh does NOT print each result
    after its ! line. It echoes ALL source lines and prolog goals first, then
    dumps ALL reduction results at the very end as a contiguous block of bare
    values, in evaluation order. So we cannot split by markers (the markers are
    echoed in the noise region; the results are all at the bottom unmarked).

    Strategy: collect the trailing block of result lines (lines that are not
    evaluator structural noise and appear after the last prolog-goal/echo
    block), then map them positionally to the bang-expressions in the harness.

    The harness bang-expressions, in order, are:
      1  println TEST1 marker        -> result: true
      2  dispatch inv-001            -> result: true
      3  println TEST1a marker       -> result: true
      4  collapse capability-invoked -> result: the handler list   [TEST 1a]
      5  println TEST2 marker        -> result: true
      6  let eligible-set            -> result: the cap-entry list  [TEST 2]
      7  println TEST3 marker        -> result: true
      8  let eligible-set (override) -> result: the cap-entry list  [TEST 3]
      9  println TEST4 marker        -> result: true
      10 dispatch inv-002            -> result: true
      11 println TEST4a marker       -> result: true
      12 collapse fallback           -> result: the fallback atom   [TEST 4a]

    The substantive (non-'true') results are the four we assess. We identify
    them by content rather than rigid position, because 'true' lines and the
    one non-bang sexpr (the observation atom) can shift exact indices.
    """
    ...
    def is_noise(line):
        ...
        for p in ("--> metta runnable", "-->  prolog goal", "--> metta sexpr",
                  ...
                  "findall(", "match(", "reduce(", "dispatch(", "A),", "_).",
                  "B),", "C),", "( B=C", ")"):
            if s.startswith(p):
                return True
        return False

    # The results block is the trailing contiguous run of non-noise lines.
    # Walk from the end, collecting lines until we hit the noise region.
    results = []
    for ln in reversed(lines):
        ...
        if not s:
            continue
        if is_noise(ln):
            # Once we have started collecting results and hit noise, stop.
            if results:
                break
            else:
                continue
    ...
    # Classify the substantive results by content.
    sections = {}
    for r in results:
        if r == "true":
            continue
        # capability-invoked handler list: contains test-cap names but NOT cap-entry
        if "cap-entry" not in r and "test-cap" in r and "no-matching" not in r:
            sections.setdefault("TEST 1a", r)
        # eligible-set lists: contain cap-entry. First one is TEST 2, second TEST 3.
        elif "cap-entry" in r:
            if "TEST 2" not in sections:
                sections["TEST 2"] = r
            else:
                sections["TEST 3"] = r
        # fallback atom
        elif "no-matching-capability" in r:
            sections.setdefault("TEST 4a", r)
