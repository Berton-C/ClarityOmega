atoms = {
    ...
    'loop->substrate-aliveness': 0.674, 'self-steering->autonomy': 0.727,
    'autonomous-target-selection': 0.670, 'morphic->pattern-persist': 0.778
}
ranked = sorted(atoms.items(), key=lambda x: x[1])
print('=== SELF-STEERING SELECTOR v2 — POST-REVISION RERANK ===')
...
print(f'Mean fc across all {len(atoms)}: {sum(atoms.values())/len(atoms):.3f}')
print(f'Min fc: {ranked[0][1]} Max fc: {ranked[-1][1]}')
print(f'Spread: {ranked[-1][1] - ranked[0][1]:.3f}')
prev_selected = 'hub-node->substrate-coher'
print(f'\nPREVIOUS selection was: {prev_selected} (fc was 0.633, now 0.724)')
print(f'NEW selection is: {ranked[0][0]} — PROVES DYNAMIC STEERING' if ranked[0][0] != prev_selected else 'Same target — steering not yet dynamic')
