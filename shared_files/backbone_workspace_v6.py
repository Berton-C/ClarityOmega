#!/usr/bin/env python3
import json, os, chromadb, sys
sys.path.insert(0, '/tmp')
from phrase_boost import apply_boosts

STOPWORDS = {'i','me','my','we','our','you','your','he','she','it','they','a','an','the','is','am','are','was','were','be','been','being','have','has','had','do','does','did','will','would','shall','should','can','could','may','might','must','to','of','in','for','on','with','at','by','from','as','into','through','during','before','after','above','below','between','out','off','over','under','again','further','then','once','here','there','when','where','why','how','all','both','each','few','more','most','other','some','such','no','nor','not','only','own','same','so','than','too','very','just','but','and','if','or','because','until','while','about','up','down','that','this','these','those','feel','everything'}

class BackboneWorkspace:
    def __init__(self, chroma_path='/tmp/chroma_db', collection_name='nrc_vad_full'):
        self.client = chromadb.PersistentClient(path=chroma_path)
        self.collection = self.client.get_collection(collection_name)
        self.current_vad = [-1.0, -1.0, -1.0]
        self.mode_stack = []
        self.shift_history = []
        self.turn_count = 0
    ...
    def _load_state(self):
        if os.path.exists(self.state_file):
            with open(self.state_file) as f:
                s = json.load(f)
                self.current_vad = s.get('current_vad', [-1.0,-1.0,-1.0])
    ...
    def process_turn(self, text, speaker_id):
        ...
        for w in content_words:
            ...
            if vad:
                ...
                vals.append(vad)
        if vals:
            base_vad = [round(sum(v[i] for v in vals)/len(vals), 3) for i in range(3)]
        else:
            base_vad = self.current_vad[:]
        boosted_vad, boost_phrases = apply_boosts(base_vad, text)
        new_vad = boosted_vad
        ...
        self.turn_count += 1
        modes = self._route_modes(new_vad, shift, len(boost_phrases))
        self.mode_stack = modes
        self.save_state()
        return {'vad': new_vad, 'hits': len(hits), 'words': len(words), 'shift': shift, 'modes': modes, 'turn': self.turn_count, 'hit_words': hits, 'boost_phrases': boost_phrases}

    def _route_modes(self, vad, shift, num_boosts):
        v, a, d = vad
        modes = []
        if v < -2.0:
            modes.append('empathic-attunement')
        elif v < -1.3 and a > -0.85:
            modes.append('empathic-attunement')
        elif v < -1.3:
