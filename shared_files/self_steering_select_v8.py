atoms = {
    ...
    'resonance-reward->fc-gain': 0.733, 'fc-gain->effort-realloc': 0.755,
    'web-cycle->pattern-persist': 0.754, 'hub-node->substrate-coher': 0.751,
    'fc-gain->revision-priority': 0.733, 'revision-priority->effort': 0.756,
    'effort->fc-gain-detection': 0.747, 'loop->substrate-aliveness': 0.746,
    'self-steering->autonomy': 0.753, 'autonomous-target-selection': 0.735,
...
print('=== SELECTOR v8 — MILESTONE 3 CHECK ===')
for i, (name, fc) in enumerate(ranked):
    tag = ' <0.73' if fc < 0.73 else ''
...
print(f'\nMILESTONE 3: ALL ATOMS ABOVE 0.73 = {all(v>=0.73 for v in atoms.values())}')
