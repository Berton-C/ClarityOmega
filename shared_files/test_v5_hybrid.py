...
def sense_v5(text, k=5, min_ext=0.3):
    lex = _load_lexicon()
    ...
    for w in words:
        if w in lex:
            ...
            if e >= min_ext:
                scored.append((e, v, w))
    if not scored:
        return {'emotion': 'unknown', 'dist': 0, 'key_words': [], 'vad': []}
    ...
    kw = [s[2] for s in top]
    return {'emotion': best, 'dist': round(dists[best], 4), 'key_words': kw, 'vad': [round(x,3) for x in sig.tolist()]}
...
for label, t in texts:
    r = sense_v5(t)
    print(label, '->', r['emotion'], 'dist:', r['dist'], 'words:', r['key_words'], 'vad:', r['vad'])
