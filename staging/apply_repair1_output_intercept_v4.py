...
Self-documenting log lines (per Clarity Arc-038 and Berton):
- MUTATION-GATE-PROOF: flag= ... lock= ... -- proves the native gate wrote
  &soul_mutation_lock (the silent-failure fix from the Python helper) in the runtime
  log every cycle, so the lock-write is provable without an external step.
- SOUL_VERDICT_OUT: ... ENFORCEMENT: repair-3-pending -- a real PAUSE verdict here is
  computed but not yet halting (halting is Repair 3). The annotation keeps the log from
  reading as a governance failure when PAUSE is computed pre-enforcement.
...
Usage
-----
Dry-run (default):  python3 apply_repair1_output_intercept_v4.py
Apply:              python3 apply_repair1_output_intercept_v4.py --apply
Reverse:            python3 apply_repair1_output_intercept_v4.py --reverse --apply
...
# Includes two self-documenting prints (per Clarity Arc-038 + Berton):
#   - MUTATION-GATE-PROOF: surfaces the flag and the lock state so the lock-write
#     (the silent-failure fix) is provable in the log every cycle, not an external step.
#   - ENFORCEMENT-PENDING annotation: a real PAUSE verdict here is computed-but-not-halting
#     until Repair 3; the log says so, so it does not read as a governance failure.
LOOP_NEW_VERDICT_BLOCK = (
    IND + ";; OUTPUT EVALUATION: real verdict per master doc Section 14 (soul-llm-call preserved).\n"
    + IND + '($_ (println! (MUTATION-GATE-PROOF: flag= $soul_mutation_flag lock= (get-state &soul_mutation_lock))))\n'
    ...
    + IND + '($_ (println! (SOUL_VERDICT_OUT: $soul_verdict_out ENFORCEMENT: repair-3-pending)))\n'
)
...
def loop_reverse_state_ok(content: str) -> tuple[bool, str]:
    ...
    # Symmetric with forward (Clarity Arc-039): also confirm old content is absent,
    # so a manually re-added old anchor is caught before reverse.
    old_absent = (count_sub(content, LOOP_OLD_VERDICT) == 0
                  and count_sub(content, LOOP_OLD_MUTATION_PY) == 0)
    ok = n and old_absent
    return ok, f"new blocks present={n}, old absent={old_absent} -> {'OK' if ok else 'FAIL'}"
...
def artifact1_reverse_state_ok(content: str) -> tuple[bool, str]:
    ...
    old_absent = (count_sub(content, ARTIFACT1_OLD_FLAG) == 0
                  and count_sub(content, ARTIFACT1_OLD_MUTGATE_FLAG) == 0)
    ok = n and old_absent
    return ok, f"new flags present={n}, old absent={old_absent} -> {'OK' if ok else 'FAIL'}"
