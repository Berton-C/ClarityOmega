...
D7 live probes bounded to THIS VALIDATION RUN (R1): a UTC boundary is
   captured at probe start, the validator polls docker logs --since that
   boundary until the live loop emits a fresh COUPLING-STATE line (or the
   probe timeout expires, a FAIL), and only post-boundary lines are checked
   for groundness and hold markers. D8 metrics from the same post-boundary
   window.
...
FROZEN_INSTALLED = {
    "soul/corner_gap/coupling_legibility.metta":
        "0665131e1491bced69a8ab3e5870637a6a9757b45375fee3095d17ade540cc81",
    "soul/corner_gap/coupling_legibility_writers.metta":
...
def main():
    ...
    ap.add_argument("--probe-timeout", type=int, default=120,
                    help="seconds to wait for a post-boundary live COUPLING-STATE line (D7)")
    args = ap.parse_args()
    ...
    if args.expect_reversed:
        ...
        check("R", "coupling imports absent", "./soul/corner_gap/coupling_legibility)" not in man)
        ...
        check("R", "soul files absent",
              not os.path.exists("soul/corner_gap/coupling_legibility.metta")
              and not os.path.exists("soul/corner_gap/coupling_legibility_writers.metta"))
    ...
    p1 = line_of("./soul/corner_gap/coupling_legibility)")
    p2 = line_of("./soul/corner_gap/coupling_legibility_writers)")
    ...
    if not args.skip_container:
        ...
        for rel in (ENGINE_REL, "soul/corner_gap/coupling_legibility.metta",
                    "soul/corner_gap/coupling_legibility_writers.metta"):
            harness += "\n;; ==== inline: " + rel + " ====\n" + rd(rel) + "\n"
        harness += FIXTURES
        ...
        # ------------- D7 live probes bounded to THIS VALIDATION RUN (R1) -------------
        # Boundary is captured NOW; only lines the production loop emits AFTER
        # this instant count. The poll waits for the loop to produce at least
        # one fresh cycle, proving the CURRENTLY INSTALLED code live.
        import time
        from datetime import datetime, timezone
        probe_started = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
        deadline = time.time() + max(10, args.probe_timeout)
        cl, logs = [], ""
        rc_ok = True
        while time.time() < deadline:
            rl = drun(["docker", "logs", "--since", probe_started, args.container])
            if rl.returncode != 0:
                rc_ok = False
                logs = (rl.stderr or "")
                break
            ...
            if cl:
                break
            time.sleep(5)
        check("D7", "docker logs --since rc==0", rc_ok, logs[:120] if not rc_ok else "")
        check("D7", "post-boundary COUPLING-STATE line produced by the LIVE loop",
              len(cl) > 0,
              "%d lines after %s (timeout %ds)" % (len(cl), probe_started, args.probe_timeout))
        check("D7", "zero held-by-corner-gate in post-boundary lines",
        ...
        if cl:
            ...
            check("D7", "ground-symbol render check on post-boundary lines", ground)
        else:
            check("D7", "ground-symbol render check on post-boundary lines", False,
                  "no post-boundary lines to check")

        # ------------- D8 metrics from the SAME post-boundary window -------------
        ...
        metrics = {"probe_started": probe_started, "lines": len(cl),
        ...
        check("D8", "observation metrics written (post-boundary window)", True,
              json.dumps(metrics))
