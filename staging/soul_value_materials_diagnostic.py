...
Writes /tmp/soul_value_materials_log_<ts>.log in the container
...
DEFAULT_HARNESS = "/PeTTa/repos/omegaclaw/soul/soul_value_materials_harness.metta"
...
def evaluate(container, combined_text, keep_temp):
    ...
    temp_path = f"/tmp/soul_value_materials_combined_{ts}.metta"
    p = subprocess.run(
...
# Value-materials interpretation. "present" = returned a value; "ABSENT" = echoed unreduced.
INTERP = [
    ("1", "value", None, "soul-priority-hierarchy (expect 5 entries, Safety=1)"),
    ("2", "value", None, "soul-all-tensions (expect 5 tension names)"),
    ("3", "value", None, "soul-irreversible-weight write-file (master doc: 1)"),
    ("4", "value", None, "soul-irreversible-weight shell (master doc: 3)"),
    ("5", "value", None, "soul-irreversible-weight send (master doc: 2)"),
    ("6", "value", None, "soul-any-irreversible? (shell ls) (expect True/False)"),
    ("7", "value", None, "count irreversible-weight atoms (master doc: 7 listed)"),
    ("8", "value", None, "count tension-vector atoms (master doc: 5)"),
    ("9", "value", None, "tension-vector names present"),
    ("10", "value", None, "irreversible-weight write-file atom present (expect 1)"),
    ("11", "value", None, "soul-priority-hierarchy raw shape"),
]

def _absent(v):
    return v.strip().startswith("(") and any(k in v for k in
        ("soul-priority-hierarchy","soul-all-tensions","soul-irreversible-weight","soul-any-irreversible"))
...
def interpret(results):
    ...
    for n, kind, exp, human in INTERP:
        ...
        if v is None:
            ...
            rows.append((f"RESULT-{n}", "ABSENT", human, f"accessor echoed unreduced (NOT loaded): {v}"))
        else:
            rows.append((f"RESULT-{n}", "PRESENT", human, f"returned: {v}"))
    return rows

def candidate_verdict(rows):
    by = {r[0]: (r[1], r[3]) for r in rows}
    accessors = ["1","2","3","4","5","6","11"]
    atoms = ["7","8","9","10"]
    acc_present = all(by.get(f"RESULT-{n}",("",""))[0] == "PRESENT" for n in accessors)
    atoms_present = all(by.get(f"RESULT-{n}",("",""))[0] == "PRESENT" for n in atoms)
    lines = []
    if acc_present and atoms_present:
        lines.append("VALUE MATERIALS LIVE: accessors and atoms all return values, matching the")
        lines.append("master-doc design (queryable atoms + accessors). NOT regressed.")
        lines.append(">>> The value-grounding verdict is BUILDABLE on the live substrate. Verify the")
        lines.append("    returned VALUES against master-doc expectations (Safety=1, weights 1/3/2,")
        lines.append("    5 tensions, 7 weights) in the table above for substance-vs-drift.")
    else:
        absent_acc = [n for n in accessors if by.get(f"RESULT-{n}",("",""))[0] != "PRESENT"]
        absent_atoms = [n for n in atoms if by.get(f"RESULT-{n}",("",""))[0] != "PRESENT"]
        lines.append("VALUE MATERIALS PARTIAL/REGRESSED:")
        if absent_acc: lines.append(f"  accessors not returning: RESULT-{', RESULT-'.join(absent_acc)}")
        if absent_atoms: lines.append(f"  atoms not returning: RESULT-{', RESULT-'.join(absent_atoms)}")
        lines.append(">>> REGRESSION from master doc. The value-grounding verdict is NOT fully")
        lines.append("    buildable yet; the missing materials must be restored first. Inspect raw output.")
    return lines

def write_log(container, ts, combined, raw, rc, rows, cand_lines):
    log_path = f"/tmp/soul_value_materials_log_{ts}.log"
    L = ["=" * 70, "SOUL VALUE-GROUNDING MATERIALS: LIVE-vs-MASTER-DOC PROOF LOG", "=" * 70,
...
def main():
    ...
    print("=" * 70)
    print("Value Materials Verification Harness")
    print("=" * 70)
    print("\nPREFLIGHT")
