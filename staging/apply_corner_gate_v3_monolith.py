...
Payloads: A.4 (pure and helper unchanged from A.3), hashes FROZEN below.
...
# ---------------- FROZEN A.5 CANDIDATE PAYLOAD IDENTITY (ratified 2026-07-10) --------
FROZEN = {
...
PURE_DST = "soul/corner_gap/coupling_legibility.metta"
WRITERS_DST = "soul/corner_gap/coupling_legibility_writers.metta"
BAK = ".bak.v3_monolith"
...
NEW_IMPORTS = (MARK_B + "\n"
               "!(import! &self (library omegaclaw ./soul/corner_gap/coupling_legibility))\n"
               "!(import! &self (library omegaclaw ./soul/corner_gap/coupling_legibility_writers))\n"
               + MARK_E)
...
def installed_helper_body_hash(helper_text):
    """Hash of the installed marked helper body, normalized to reconstruct the
    exact reviewed payload bytes (installer stores payload.rstrip newline
    between the markers)."""
    try:
...
def main():
    ...
    ap.add_argument("--inject-postflight-fail", action="store_true",
                    help="TEST HOOK: force a postflight failure to prove auto-reverse")
    ap.add_argument("--self-test-mode", action="store_true",
                    help="mock-tree self-test gate; REQUIRED for any pin override")
    ap.add_argument("--expected-engine-sha", default=None,
                    help="override the pinned engine sha; refused without --self-test-mode")
    ...
    os.chdir(root)
    if args.expected_engine_sha and not args.self_test_mode:
        print("REFUSED: --expected-engine-sha requires --self-test-mode (R2 hardening).")
        print("Production runs verify the canonical pinned engine only.")
        sys.exit(3)
    ...
    if args.self_test_mode:
        say("!! SELF-TEST MODE: canonical substrate identity NOT verified !!")
    say("=" * 72)
    ...
    check("SUBSTRATE IDENTITY: engine sha matches pinned canonical", engine_sha_pre == pin,
          "have " + engine_sha_pre[:12] + " want " + pin[:12])
    ...
    p1 = line_of3("./soul/corner_gap/coupling_legibility)")
    p2 = line_of3("./soul/corner_gap/coupling_legibility_writers)")
    ...
    check("B2: installed helper body equals frozen reviewed payload",
          installed_helper_body_hash(rd(HELPER_REL)) == FROZEN["coupling_legibility_helper_payload.py"])
    if args.inject_postflight_fail:
        check("INJECTED POSTFLIGHT FAILURE (test hook)", False, "proving auto-reverse")
    ...
    if FAILED:
        ...
        do_reverse(False, exit_code_on_success=3)
...
def do_reverse(dry, exit_code_on_success=0):
    """Reverse with ISOLATED failure state (B1): the apply run's global
    FAILED must not poison reverse preflight. Verification is hash-level
    (M3): every restored file compares against the recorded pre-apply hash
    when the apply manifest exists."""
    rfails = []
    ...
    for rel in (MANIFEST_REL, LOOP_REL, HELPER_REL, GATE_REL, ART1_REL):
        b = rel + BAK
        rcheck("backup present " + b, os.path.exists(b))
        restore.append((b, rel))
    cwwr = CWW_REL + ".retired.v3"
    rcheck("retired file present", os.path.exists(cwwr) or os.path.exists(CWW_REL + BAK) or os.path.exists(CWW_REL))
    if rfails:
    ...
    if dry:
        ...
        return
    for b, rel in restore:
        shutil.copy2(b, rel)
    if os.path.exists(cwwr):
        shutil.move(cwwr, CWW_REL)
    elif not os.path.exists(CWW_REL) and os.path.exists(CWW_REL + BAK):
        shutil.copy2(CWW_REL + BAK, CWW_REL)
    for rel in (PURE_DST, WRITERS_DST):
        if os.path.exists(rel):
            os.remove(rel)

    say("REVERSE VERIFICATION (hash-level where the apply manifest exists):")
    pre = {}
    mpath = "staging/v3_monolith_apply_manifest.json"
    if os.path.exists(mpath):
        try:
            with open(mpath) as f:
                pre = json.load(f).get("pre_hashes", {})
        except Exception:
            pre = {}
    for rel in (MANIFEST_REL, LOOP_REL, HELPER_REL, GATE_REL, ART1_REL):
        if rel in pre:
            rcheck("restored hash matches pre-apply " + rel, sha(rel) == pre[rel])
    if CWW_REL in pre:
        rcheck("restored hash matches pre-apply " + CWW_REL, sha(CWW_REL) == pre[CWW_REL])
    else:
        rcheck("retired writer restored " + CWW_REL, os.path.exists(CWW_REL))
    man = rd(MANIFEST_REL)
    loop = rd(LOOP_REL)
    rcheck("imports gone", "./soul/corner_gap/coupling_legibility)" not in man and MARK_B not in man)
    rcheck("corner_window_writers import restored", CWW_IMPORT in man)
    rcheck("hook restored", HOOK_OLD_SUB in loop and HOOK_NEW_SUB not in loop)
    rcheck("bootstrap hook gone", "(do-bootstrap-coupling!)" not in loop)
    rcheck("helper payload gone", PY_MARK_B not in rd(HELPER_REL))
    rcheck("artifact_1 section gone", ART_MARK_B not in rd(ART1_REL))
    rcheck("created soul files removed",
           not os.path.exists(PURE_DST) and not os.path.exists(WRITERS_DST))
    rcheck("v2 gate body restored (held markers present again)",
           "held-by-corner-gate" in rd(GATE_REL))
    rcheck("engine untouched through reverse", os.path.exists(ENGINE_REL))
    if rfails:
        say(">>> REVERSE FAILED: " + "; ".join(rfails))
        sys.exit(3)
    say(">>> REVERSE COMPLETE AND HASH-VERIFIED.")
    sys.exit(exit_code_on_success)
