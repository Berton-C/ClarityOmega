#!/usr/bin/env python3
"""
quantale_v08_5_canonical_integration_harness.py
...
class Recorder:
    ...
    def raw_block(self, title: str, text: str, tail: int = 220) -> None:
        self.lines.append(f"### {title}")
...
def runtime_probe(engine_path: Path, container: str, probe_set: str, rec: Recorder) -> None:
    rec.section(f"Tier 7 - Runtime reduction probe set: {probe_set}")
    ...
    for _, expr, _ in probes:
        combined += expr + "\n"
    write_cmd = ["docker", "exec", "-i", container, "sh", "-c", "cat > /tmp/_v08_5_canonical_probe.metta"]
    rc, out, err = run_cmd(write_cmd, input_text=combined)
    if rc != 0:
        rec.add("Tier 7", "write v08.5 probe to container", "FAIL", err.strip())
        return
    rc, out, err = run_cmd(["docker", "exec", container, "sh", "-c", "cd /PeTTa && ./run.sh /tmp/_v08_5_canonical_probe.metta 2>&1"], timeout=180)
    raw = out + err
    rec.raw_block("Raw v08.5 MeTTa probe output tail", raw)
    if rc != 0:
        rec.add("Tier 7", "run v08.5 MeTTa probe", "FAIL", f"run.sh exited {rc}")
        return
    for name, _, expected in probes:
        rec.add("Tier 7", name, "PASS" if expected in raw else "FAIL", f"expected token={expected}")
...
def main() -> int:
    ...
    if args.runtime:
        runtime_probe(engine_path, args.container, args.probe_set, rec)
    else:
    ...
    md, js = rec.write({
        "harness_version": "v08.5-canonical-integration-v01",
        "cwd": str(Path.cwd()),
