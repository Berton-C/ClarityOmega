...
Writes /tmp/soul_lock_structural_log_<ts>.log in the container
...
DEFAULT_HARNESS = "/PeTTa/repos/omegaclaw/soul/soul_lock_structural_harness.metta"
...
def evaluate(container, combined_text, keep_temp):
    ...
    temp_path = f"/tmp/soul_lock_structural_combined_{ts}.metta"
    p = subprocess.run(
...
# Structural lock interpretation.
INTERP = [
    ("1", "true",  None, "clear state reads back == unlocked -> True"),
    ("2", "false", None, "pending? when clear -> False"),
    ("3", "true",  None, "pending? when held -> True"),
    ("4", "value", None, "get-state when held -> (locked soul-note)?"),
    ("5", "false", None, "pending? after reclear -> False"),
    ("6", "false", None, "defined predicate: pending? clear -> False"),
    ("7", "true",  None, "defined predicate: pending? held -> True"),
    ("8", "false", None, "defined predicate: pending? recleared -> False"),
]
...
def candidate_verdict(rows):
    ...
    works = all(by.get(f"RESULT-{t}")=="PASS" for t in ("1","2","3","5","6","7","8"))
    lines = []
    if works:
        lines.append("STRUCTURAL ATOM-LOCK: WORKS end to end. clear/held/reclear all discriminate")
        lines.append("via structural == against the unlocked symbol. No string ops.")
        lines.append(">>> USE THIS for soul-mutation-pending?: (if (== (get-state &soul_mutation_lock) unlocked) False True).")
        lines.append(">>> Lock writer: clear -> unlocked ; held -> (locked <arg>). Replaces the dead string_length form.")
    else:
        fails = [r[0] for r in rows if r[1] in ("FAIL","NO-RESULT")]
        lines.append(f"STRUCTURAL ATOM-LOCK: not all green: {', '.join(fails)}. Inspect raw output.")
    return lines


def write_log(container, ts, combined, raw, rc, rows, cand_lines):
    log_path = f"/tmp/soul_lock_structural_log_{ts}.log"
    L = ["=" * 70, "SOUL STRUCTURAL LOCK: PROOF LOG", "=" * 70,
