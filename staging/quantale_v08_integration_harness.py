...
def static_validate(engine_path: Path, ladder_path: Path, rec: Recorder) -> Tuple[str, str]:
    ...
    rec.add("Tier 7", "q-suspicion-cap catch-all/default absent", "INSPECT" if not re.search(r"q-suspicion-cap \(mk-qsuspicion \$target \(mk-pbit \$s \$c\) \$", engine) else "PASS", "new grounding types may fail without explicit default")
    rec.add("Tier 7", "q-contact-refresh uses min contact-strength/contact-confidence", "INSPECT" if "(max $fconf (min $cs $cconf))" in engine else "PASS", "semantics worth deciding: conservative support gate vs warrant refresh")
    rec.add("Tier 7", "qclaim formal-proof exemption absent", "INSPECT" if "qclaim-mul" in engine and "formal-proof" in engine and "qclaim-formal-proof" not in engine else "PASS", "sequential verified proofs may be capped with inferred-style dynamics")
...
def main() -> int:
    ...
    md, js = rec.write({
        "harness_version": "v08-integration-v01",
        "cwd": str(Path.cwd()),
