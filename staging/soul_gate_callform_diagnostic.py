...
Writes /tmp/soul_gate_callform_log_<ts>.log in the container
...
DEFAULT_HARNESS = "/PeTTa/repos/omegaclaw/soul/soul_gate_callform_harness.metta"
...
def build_combined(container, harness_path):
    ...
    harn, e2 = cat(harness_path)
    if harn is None:
        return None, f"could not read harness: {e2}"
    combined = (
...
def evaluate(container, combined_text, keep_temp):
    ...
    temp_path = f"/tmp/soul_gate_callform_combined_{ts}.metta"
    p = subprocess.run(
...
# Call-form interpretation: report each form's raw value, flag which reduce correctly.
INTERP = [
    ("1", "value", None, "extract A: bare (metta (add-atom..)) -> ? (expect WRONG, add-atom executes)"),
    ("2", "value", None, "extract B: (quote (metta ..)) -> ? (pattern may not match)"),
    ("3", "value", None, "extract C: (metta (quote (add-atom..))) -> ?"),
    ("4", "value", None, "targets D: bare -> ? (expect WRONG, add-atom executes)"),
    ("5", "value", None, "targets E: (quote (add-atom..)) at call site -> expect true"),
    ("6", "value", None, "target-head F: on (quote (add-atom..)) -> expect soul-note"),
    ("7", "value", None, "full proven chain G -> expect true"),
]
...
def interpret(results):
    ...
    for n, kind, exp, human in INTERP:
        v = results.get(n)
        rows.append((f"RESULT-{n}", "VALUE" if v is not None else "NO-RESULT", human,
                     (f"returned: {v}" if v is not None else "tag not found")))
    return rows
...
def candidate_verdict(rows):
    ...
    for r in rows:
        if r[1] == "VALUE":
            vals[r[0]] = r[3].replace("returned: ", "").strip()
    lines = []
    lines.append("EXTRACT correct form: " + (
        "B (quote whole)" if vals.get("RESULT-2","").startswith("(add-atom") or "add-atom" in vals.get("RESULT-2","")
        else "C (inner quote)" if "add-atom" in vals.get("RESULT-3","")
        else "NONE clean, inspect raw"))
    targ5 = vals.get("RESULT-5","").lower()
    th6 = vals.get("RESULT-6","")
    g7 = vals.get("RESULT-7","").lower()
    lines.append("TARGETS-NAMESPACE correct form: " + (
        "E (quote at call site) -> " + targ5 if targ5 in ("true","false")
        else "inspect; target-head F returned " + th6))
    lines.append("FULL CHAIN G: " + (g7 if g7 else "no result"))
    lines.append(">>> Use whichever EXTRACT and TARGETS form reduced correctly; update the full-gate harness call sites to match, AND note the production wiring must pass the metta expression unevaluated (quoted) the same way.")
    return lines


def write_log(container, ts, combined, raw, rc, rows, cand_lines):
    log_path = f"/tmp/soul_gate_callform_log_{ts}.log"
    L = ["=" * 70, "SOUL GATE CALL-FORM: PROOF LOG", "=" * 70,
