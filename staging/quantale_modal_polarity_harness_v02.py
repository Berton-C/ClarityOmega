#!/usr/bin/env python3
"""
quantale_modal_polarity_harness_v02.py

Cold validation harness v02 for:
  - quantale_engine_tfs_v03_grounding_audit.metta
  - quantale_engine_soul_aligned_modal_polarity_audit_v01.metta

v02 extends v01 with adversarial and scenario-shaped checks:
...
class Recorder:
    ...
    def write(self, meta: dict) -> Tuple[Path, Path]:
        ...
        md = self.out_dir / f"modal_polarity_validation_trace_v02_{self.stamp}.md"
        js = self.out_dir / f"modal_polarity_validation_trace_v02_{self.stamp}.json"
        summary = summarize(self.checks)
        header = [
            "# Modal Polarity Validation Trace v02",
            "",
...
def main(argv: List[str] | None = None) -> None:
    ...
    meta = {"cwd": str(base), "tfs": str(tfs_path), "modal": str(modal_path), "container": args.container, "runtime": args.runtime, "harness_version": "v02"}
