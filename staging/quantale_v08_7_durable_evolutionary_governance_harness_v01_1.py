...
VERSION = "v08.7-durable-evolutionary-governance-v01.1"
...
def inspect_governance(root: Path, manifest: Optional[str]) -> List[Check]:
    ...
    if not ktxt:
        ...
        return checks
    checks.append(Check("G0", "soul kernel readable", "PASS", f"path={kernel} class_atoms={len(class_atoms)} classes={classes}",
                        {"class_atoms": class_atoms[:20], "classes": classes}))
    checks.append(Check("G1", "durable.metta journal class declaration", "PASS" if durable_line in ktxt else "HOLD",
                        "present" if durable_line in ktxt else f"missing expected class declaration for {durable_abs}"))
    checks.append(Check("G1", "soul file class default-deny visible", "PASS" if "runtime-soul" in ktxt and "soul-file-class-of" in ktxt else "HOLD",
    ...
    for pat in ["**/lib_clarity_reasoning*.metta", "**/*manifest*.metta", "**/*reasoning*.metta"]:
    ...
    for p in candidates[:100]:
        ...
        if "durable.metta" in txt or durable_abs in txt:
            import_hits.append(str(p))
    checks.append(Check("G2", "durable.metta import-chain reference", "PASS" if import_hits else "HOLD",
                        f"import_hits={import_hits[:10]}" if import_hits else "no durable.metta import reference found in searched manifest candidates",
                        {"searched_count": len(candidates), "import_hits": import_hits[:20]}))
    return checks


def validate_serialization_file(path: Path) -> Tuple[str, Dict[str, Any]]:
    if not path.exists():
        return "HOLD", {"path": str(path), "exists": False}
    bad=[]
    checked=0
    for i,line in enumerate(path.read_text(errors="replace").splitlines(),1):
        ...
        if verdict != "serialization-valid":
            bad.append({"line": i, "verdict": verdict, "text": s[:200]})
    return ("PASS" if not bad else "FAIL"), {"path": str(path), "checked_lines": checked, "bad": bad[:20]}
