#!/usr/bin/env python3
"""Repair 3, Surfaces A+B (re-land) + the string-contains fix (F-R3-7).

THE FIX (F1, soul_utils.metta, Clarity's file, her sign-off rides the
evidence message): string-contains was ALWAYS-TRUE since Stage 3
(string-replace mis-reduces to O: probes 10/11/12). All three routing
predicates (soul-pause?/flag?/proceed?) lied; input soul-note-record never
fired; the substrate router misfire that reverted the first A+B landing
was this lie given consequences. Native scan ops exhausted by probe12
census (partial/self-return); fix = py-bridge scan via the
production-proven repr_kind pattern. BOTH-POLARITY fixtures throughout
(C6: an always-True function aces a True-only suite; an always-False
candidate aces a False-only suite; both happened this session).

RE-LAND retires W1's halt half and W6 at wiring level.
...
UTILS = Path("soul/soul_utils.metta")

# ---- F1: the string-contains fix (F-R3-7; Clarity sign-off on her file) ----
F1_OLD = """;; string-contains: True if $haystack contains $needle
(= (string-contains $haystack $needle)
   (not (== (string-replace $haystack $needle "") $haystack)))"""
F1_NEW = """;; string-contains: True if $haystack contains $needle
;; Repair 3 fix (2026-06-11, F-R3-7, probes 10/11/12): string-replace
;; mis-reduces (to the constant O), making the old body ALWAYS-TRUE since
;; Stage 3. Native scan ops are partial/absent in this runtime (probe12
;; census), so the scan delegates to the production-proven bridge pattern
;; (repr_kind shape: string in, int out, == n 1). Policy stays in this body.
(= (string-contains $haystack $needle)
   (== (py-call (soul_governance.contains_token $haystack $needle)) 1))"""
...
def main():
    ...
    l, g, art, u = LOOP.read_text(), GOV.read_text(), ART1.read_text(), UTILS.read_text()
    try:
        if not a.reverse:
            u2 = apply_pairs(u, [("F1", F1_OLD, F1_NEW)])
            l2 = apply_pairs(l, EDITS_LOOP)
            if B5_MARK in g: raise RuntimeError("B5 already applied")
            g2 = g.rstrip("\n") + B5_CODE
            art2 = apply_pairs(art, EDITS_ART1)
        else:
            u2 = apply_pairs(u, [("F1", F1_OLD, F1_NEW)], reverse=True)
    ...
    ld = {"soul_utils.metta": (len(u2.splitlines()) - len(u.splitlines()), 5 * direction),
          "loop.metta": (len(l2.splitlines()) - len(l.splitlines()), 20 * direction),
    ...
    if not a.reverse:
        ok2 = verify_gov(g2)
        ns = {}
        exec(g2[g2.find("def contains_token"):g2.find("def pause_context")], ns)
        ct = ns["contains_token"]
        ok3 = (ct("say VERDICT: PAUSE now", "VERDICT: PAUSE") == 1
               and ct("VERDICT: PROCEED", "VERDICT: PAUSE") == 0
               and ct("PATTERNS Safety: No gap. VERDICT: PROCEED SOUL-TONE: honest", "VERDICT: PAUSE") == 0
               and ct("VERDICT: PAUSE SOUL-NOTE: x", "VERDICT: PAUSE") == 1)
        print(f"FUNCTION VERIFY pause helpers (compose/context/empty): {'PASS' if ok2 else 'FAIL'}")
        print(f"FUNCTION VERIFY contains_token (BOTH polarities x2, C6): {'PASS' if ok3 else 'FAIL'}")
        if not (ok2 and ok3): return 1
    ...
    if not a.reverse:
        for p, orig in ((LOOP, l), (GOV, g), (ART1, art), (UTILS, u)):
            Path(str(p) + ".bak.repair3").write_text(orig)
        print("Backups: *.bak.repair3 x4")
    LOOP.write_text(l2); GOV.write_text(g2); ART1.write_text(art2); UTILS.write_text(u2)
    do, dc = code_aware_parens(LOOP.read_text())
    print(f"DISK RECHECK: loop.metta parens {do}/{dc} delta {do-dc}")
    print(f"Wrote 4 files ({'REVERSE' if a.reverse else 'FORWARD'})")
    print("LIVE CANARY after rebuild: iteration 1 must NOT pause (the old bug")
    print("falsely PAUSEd every boot on the PROCEED seed; silence = proof).")
