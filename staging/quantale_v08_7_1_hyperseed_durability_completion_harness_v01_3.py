...
VERSION = "v08.7.1-hyperseed-durability-completion-v01.3"
...
REQUIRED_FAMILIES = [
    ...
    "q-v08-7-1-hyperseed-field-status?",
    "q-v08-7-1-durable-growth-threshold?",
    "q-v08-7-1-import-candidate-status?",
    "q-v08-7-1-support-evidence-role?",
    "q-v08-7-1-negative-control?",
]
...
SEMANTIC_PROBES: List[Tuple[str, str, str]] = [
    ...
    ("hyperseed threshold pass", "!(q-v08-7-1-durable-growth-threshold? context-valid pbit-evidence-valid proto-time-valid structural-signature-valid continuity-valid artifact-lineage-valid resource-cost-valid approximation-valid)", "hyperseed-durability-threshold-pass"),
    ("hyperseed threshold context blocked", "!(q-v08-7-1-durable-growth-threshold? blocked-context-missing pbit-evidence-valid proto-time-valid structural-signature-valid continuity-valid artifact-lineage-valid resource-cost-valid approximation-valid)", "blocked-context-missing"),
    ("hyperseed import candidate ready", "!(q-v08-7-1-import-candidate-status? v08-7-semantic-green governance-green hyperseed-threshold-pass)", "import-candidate-ready"),
    ("hyperseed governance open hold", "!(q-v08-7-1-import-candidate-status? v08-7-semantic-green governance-open hyperseed-threshold-pass)", "hold-governance-not-green"),
    ("hyperseed file survival negative", "!(q-v08-7-1-negative-control? file-survival durable-growth-claimed)", "blocked-file-survival-not-structural-durability"),
    ("hyperseed contextless negative", "!(q-v08-7-1-negative-control? contextless-claim durable-growth-claimed)", "blocked-context-missing"),
    ("hyperseed cross context copy negative", "!(q-v08-7-1-negative-control? cross-context-copy transfer-claimed)", "blocked-copy-without-resonance"),
]
...
def run_docker_probe(container: str, engine: Path, expr: str, expected: str,
                     idx: int, name: str, raw_dir: Path, raw_mode: str, tail_chars: int) -> Tuple[bool, Dict[str, Any]]:
...
def main() -> int:
    ...
    if ns.runtime:
        ...
        for idx,(name,expr,expected) in enumerate(SEMANTIC_PROBES,1):
            ok,data=run_docker_probe(ns.container, engine, expr, expected, idx, name, raw_dir, ns.raw_mode, ns.raw_tail_chars)
...
def write_report(log_dir: Path, checks: List[Check], meta: Dict[str, Any]) -> int:
    ...
    lines=["# v08.7.1 Hyperseed Durability Completion Harness Trace", "", f"harness: `{VERSION}`", "", "## Summary", "", "```json", json.dumps(summary, indent=2, sort_keys=True), "```", "", "## Checks", ""]
    for c in checks:
