#!/usr/bin/env python3
"""
quantale_v08_5_canonical_integration_harness_v02.py
...
Purpose:
  ...
  - v02 forensic runtime mode records every probe expression, command, full raw stdout/stderr, result section, unreduced status, and local token match
...
def result_section(raw: str) -> List[str]:
    ...
    for ln in seg:
        ...
        if not s:
            continue
        # goal-success noise; keep explicit True/False tokens if rendered as result
        if s == 'true':
            continue
        out.append(s)
    return out
...
def is_unreduced(raw: str, expr: str) -> bool:
    ...
    if not head:
        return False
    return any(re.search(r'\(\s*' + re.escape(head) + r'\b', ln) for ln in result_section(raw))
...
def runtime_probe(engine_path: Path, container: str, probe_set: str, rec: Recorder, stop_on_fail: bool = False) -> None:
    """Run each runtime probe in isolation and record full forensic evidence.
    ...
    v02 improvement beyond examples:
      - every JSON check carries exact expression, return code, full stdout/stderr,
        parsed result section, unreduced flag, expected-token-in-result flag, and
        expected-token-outside-result flag to expose false positives.
    """
    rec.section(f"Tier 7 - Runtime reduction probe set: {probe_set} (v02 forensic per-probe)")
    ...
    rec.add("Tier 7", "runtime forensic mode active", "PASS", f"probes={len(probes)} engine_sha256={engine_hash}; each probe is isolated and full raw output is recorded")
    ...
    for i, (name, expr, expected) in enumerate(probes, 1):
        ...
        if rcw != 0:
            data = {
                ...
                "write_stdout_full": outw,
                "write_stderr_full": errw,
            }
        ...
        rec.full_raw_block(f"FULL RAW OUTPUT probe {i:03d}: {name}", strip_ansi(raw))
...
def main() -> int:
    ...
    if args.runtime:
        runtime_probe(engine_path, args.container, args.probe_set, rec, stop_on_fail=args.stop_on_fail)
    else:
    ...
    md, js = rec.write({
        "harness_version": "v08.5-canonical-integration-v02-forensic",
        "cwd": str(Path.cwd()),
