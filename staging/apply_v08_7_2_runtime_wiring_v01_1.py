...
def insert_lib_block_forward(text: str) -> str:
    # Idempotent/partial-recovery behavior: if the managed block is already
    # present, leave it unchanged. This lets a failed earlier apply continue
    # by adding the missing kernel block without duplicating imports.
...
def insert_lib_block_reverse(text: str) -> str:
    # Idempotent reverse: absence of the managed block is already the desired state.
...
def ensure_writable(path: Path) -> bool:
    try:
        ...
        print(f"ERROR: {path} is not writable. Fix permissions before --apply, e.g.: chmod u+w {path}")
        return False
...
def write_with_backup(path: Path, bak: Path, new_text: str) -> None:
    ...
    tmp = path.with_suffix(path.suffix + ".tmp.v08_7_2_wiring")
...
def main() -> int:
    ...
    changed_paths = []
    if lib_new != lib_old:
        changed_paths.append(LIB_CR_PATH)
    if kernel_new != kernel_old:
        changed_paths.append(SOUL_KERNEL_PATH)
    for path in changed_paths:
        if not ensure_writable(path):
            print("APPLY PREFLIGHT FAILED: no files were written in this run.")
            return 1
    if not changed_paths:
        print("No file content changes needed; managed wiring is already in requested state.")

    print("\n========== WRITING ==========")
    if lib_new != lib_old:
        write_with_backup(LIB_CR_PATH, LIB_CR_BAK, lib_new)
    else:
        print(f"  unchanged {LIB_CR_PATH}")
    if kernel_new != kernel_old:
        write_with_backup(SOUL_KERNEL_PATH, SOUL_KERNEL_BAK, kernel_new)
    else:
        print(f"  unchanged {SOUL_KERNEL_PATH}")
