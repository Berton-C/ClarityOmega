...
**Version:** v1 Phase 1 (April 30, 2026)
...
| `&loops` | `(maxNewInputLoops)` | Iteration counter for run cycle | Line 54, line 62 | Line 54, line 62, line 154 |
...
2. **git-import** - Pulls fresh OmegaClaw upstream from GitHub. **Important:** This re-clones on every startup, overwriting local changes if the working code is in `/PeTTa/repos/omegaclaw/` and gets refreshed. [gap flag: needs verification of whether git-import actually overwrites or only initializes-if-missing]
...
- soul/identity_kernel: priority-rank atoms (1-5), tension-vector atoms (5), paraconsistency-pair atoms (4), irreversible-weight atoms (4)
- soul/latch/aliveness_state_machine: `(latch-state IDLE)` - the initial latch state
...
- Whether git-import in run.metta actually overwrites the working directory or only initializes-if-missing.
