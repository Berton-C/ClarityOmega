...
HARNESS_VERSION = "v08.6-hyperseed-durable-memory-v01"
REQUIRED_TOKENS = [
...
def parse_result_section(stdout:str, expression:str):
    lines=stdout.splitlines(); start=None
    for i,line in enumerate(lines):
        if line.strip().startswith("--> metta runnable"): start=i
    if start is None:
        for i,line in enumerate(lines):
            if expression.strip() in line: start=i
    if start is None: return [],"",False
    caret=None
    for i in range(start,len(lines)):
        s=lines[i].strip()
        if s and set(s)=={"^"} and len(s)>=5: caret=i
    result_lines=lines[caret+1:] if caret is not None else lines[start+1:]
    noise=("--> metta function","--> prolog clause","--> metta sexpr","--> metta runnable","-->  prolog goal",":- findall","^^^^^^^^","'","(= ","(q-")
    clean=[]
    for raw in result_lines:
        ...
        if s in {"_)."}: continue
        clean.append(s)
    actual="\n".join(clean).strip(); unreduced="(partial " in actual or actual.startswith("(partial")
    return clean,actual,unreduced
