#!/usr/bin/env python3
"""
quantale_v08_5_canonical_integration_harness_v03_compact_forensic.py
...
def runtime_probe(engine_path: Path, container: str, probe_set: str, rec: Recorder, stop_on_fail: bool = False, raw_mode: str = 'fail', raw_tail_chars: int = 12000) -> None:
    ...
    for i, (name, expr, expected) in enumerate(probes, 1):
        ...
        data = {
            ...
            "stdout_tail": tail_text(outr, raw_tail_chars),
            "stderr_tail": tail_text(errr, raw_tail_chars),
...
def main() -> int:
    ...
    ap.add_argument("--raw-tail-chars", type=int, default=12000, help="Max characters of stripped stdout/stderr tail stored in JSON/Markdown per probe.")
    args = ap.parse_args()
    ...
    md, js = rec.write({
        "harness_version": "v08.5-canonical-integration-v03-compact-forensic",
        "cwd": str(Path.cwd()),
