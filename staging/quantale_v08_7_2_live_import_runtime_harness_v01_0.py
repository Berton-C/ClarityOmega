...
- Semantic probes are query-only files executed through /PeTTa/run.sh.
- Therefore reductions pass only if the live boot/import graph already imports
  the v08.7.2 engine and the live soul/evolutionary + soul/durable files.

It may still use /tmp for the temporary query files only.
"""
...
VERSION = "v08.7.2-live-import-runtime-v01.0-query-only"
...
def make_probe_name(idx: int, name: str) -> str:
    safe = re.sub(r"[^A-Za-z0-9_.-]+", "_", name).strip("_")[:80]
    return f"/tmp/_v08_7_2_live_import_{idx:03d}_{safe}.metta"


def run_query_probe(container: str, idx: int, name: str, expr: str, expected: str, raw_dir: Path, raw_mode: str, tail_chars: int) -> Check:
    probe_path = make_probe_name(idx, name)
    body = ";; v08.7.2 live import query-only probe\n"
...
def main() -> int:
    ...
    for name, expr, expected in LIVE_TOPOLOGY_PROBES:
        checks.append(run_query_probe(ns.container, idx, name, expr, expected, raw_dir, ns.raw_mode, ns.raw_tail_chars)); idx += 1

    for name, expr, expected in SEMANTIC_PROBES:
        checks.append(run_query_probe(ns.container, idx, f"live semantic: {name}", expr, expected, raw_dir, ns.raw_mode, ns.raw_tail_chars)); idx += 1
...
def write_report(log_dir: Path, checks: List[Check], meta: Dict[str, Any]) -> int:
    ...
    lines = ["# v08.7.2 Live Import Runtime Harness Trace", "", f"harness: `{VERSION}`", "", "## Summary", "", "```json", json.dumps(summary, indent=2, sort_keys=True), "```", "", "## Checks", ""]
    for c in checks:
