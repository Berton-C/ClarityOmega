...
inductive Provenance
  ...
  | contact (seal : ContactSeal Event Contact Route)
...
inductive DirectContact
  ...
  | intro (seal : ContactSeal Event Contact Route) :
      DirectContact (.contact seal)
...
structure ContactRecord
  ...
  world : World
  datum : Observation
  seal : ContactSeal Event Contact Route
  route : Route
  generated : observe route world = datum
  sealedRef : seal.contact = contact
  sealedEvent : seal.event = event
  sealedRoute : seal.route = route
...
theorem availability_has_semantic_witness
    ...
    {d : Distinction}
    {ctx : Context}
    (t : AvailabilityTransition CarriesContact CarriesFrame RetainedFor
      RestoresRoute d ctx) :
...
theorem mmNotSelfGenerated :
  ...
  intro h
  induction h with
  | formation => exact e10_ne_e12 rfl
  | assembly => exact e10_ne_e12 rfl
  | step _ edge _ => exact edge
...
def mmRecovery : AvailabilityTransition mmCarriesContact mmCarriesFrame mmRetainedFor
...
structure MattermostModelWitness where
  ...
  authority :
    Authoritative mmTime MMReverses mmDoctrine mmObserve mmRelevant mmContactWriter
      mmContactSupport mmMaterialContribution mmWitnessSupport mmInternalEdge
      mmContactEvent mmWitness .eventDelivery e12
  recovery : AvailabilityTransition mmCarriesContact mmCarriesFrame mmRetainedFor
    mmRestoresRoute .fineLatency .netToEvent
  interpretationUse :
    InterpretationUse (Scope := MMScope) (Action := MMAction)
      (SoulJudgment := MMSoulJudgment) mmInterpretation
