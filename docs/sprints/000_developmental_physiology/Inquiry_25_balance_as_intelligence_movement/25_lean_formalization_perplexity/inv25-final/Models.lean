...
inductive Movement : Cut → Cut → Type where
  ...
  /-- NEW (Correction C adversary): passes every Adm25 check via the
  existing wildcard defaults below, so that the ONLY way its
  `HarmonicAlign` witness can fail is the new universal material-extension
  clause — isolating Correction C's countermodel from every other
  obligation. -/
  | matExtMisaligned : Movement .K .K'
...
/-- `ext1`: the original well-behaved extension (still fails alignment for
`forcedHighHigh`/`lowActHighForce`, unchanged). `extBad`: a MATERIALLY
participating extension (via `ExtensionDiscriminates`) that is misaligned
specifically for `matExtMisaligned` — Correction C's positive adversary.
`extIrrelevant`: nominally "of" `skilled` (`ExtensionOf = True`, the weak
self-certifiable membership) but never MATERIAL (`ExtensionDiscriminates =
False` unconditionally) and deliberately misaligned — demonstrating that a
non-material extension's misalignment can never block `HarmonicAlign`. -/
inductive Extension where | ext1 | extBad | extIrrelevant
deriving DecidableEq
...
/-- NEW (Correction C): mirrors `DiscriminatesRel`'s role for extensions.
`ext1` discriminates always; `extBad` discriminates ONLY for
`matExtMisaligned`; `extIrrelevant` never discriminates (never material). -/
def ExtensionDiscriminates (e : Extension) {K K' : Cut} (m : Movement K K') : Prop :=
  match e with
  | .ext1 => True
  | .extBad => match m with | .matExtMisaligned => True | _ => False
  | .extIrrelevant => False
...
def AlignedExtension (e : Extension) {K K' : Cut} (m : Movement K K') : Prop :=
  ...
  | .extBad, .matExtMisaligned => False
  | .extIrrelevant, .skilled => False
  | _, _ => True
...
def OpportunityFieldUnavailable (t : Trajectory) : Prop :=
  match t with
  | .tStuck => True
  | _ => False
...
@[reducible] def sig : Sig where
  ...
  ExtensionDiscriminates := ExtensionDiscriminates
  ...
  OpportunityFieldUnavailable := OpportunityFieldUnavailable
...
/-- `matExtMisaligned` passes every other Adm25 check via the same
wildcard defaults `skilled` uses — isolating Correction C's countermodel
from every other obligation. -/
def matExtMisalignedAdm : Adm25Witness sig doctrine matExtMisaligned :=
  { available := trivial
    adm24 := trivial
    relFloors := fun r _ => rfl
    precFloors := fun d _ => rfl
...
def matExtMisalignedReadApp : ReadApp sig doctrine matExtMisaligned
    { prov := provenanceOfMovement matExtMisaligned
      gen := generationKindOf matExtMisaligned
      kind := ReadingKind.derivedAppropriateness } :=
  @ReadApp.mk sig doctrine _ _ matExtMisaligned matExtMisalignedAdm trivial

theorem m1_matExtMisaligned_never_direct_contact :
    ({ prov := provenanceOfMovement matExtMisaligned
       gen := generationKindOf matExtMisaligned
       kind := ReadingKind.derivedAppropriateness } : Appropriateness sig matExtMisaligned).kind
      ≠ ReadingKind.directContact :=
  readapp_no_contact_masquerade sig doctrine matExtMisaligned _ matExtMisalignedReadApp
...
theorem T18_pns_high_force_contrast :
    (HighActionEffort 50 (actionEffort skilled) ∧ LowExcessForcing 50 (excessForcing skilled) ∧
        ¬ DisqualifyingExcessForcing skilled ∧ LegacyLawfulAppropriate sig doctrine skilled) ∧
      (HighActionEffort 50 (actionEffort forcedHighHigh) ∧
        HighExcessForcing 50 (excessForcing forcedHighHigh) ∧
        DisqualifyingExcessForcing forcedHighHigh ∧
          ¬ LegacyLawfulAppropriate sig doctrine forcedHighHigh) :=
  ⟨⟨by unfold HighActionEffort actionEffort; decide, by unfold LowExcessForcing excessForcing; decide,
      (fun h => h), ⟨_, skilledReadApp⟩⟩,
    ⟨by unfold HighActionEffort actionEffort; decide, by unfold HighExcessForcing excessForcing; decide,
      trivial, fun ⟨A, hReadApp⟩ => by
        obtain ⟨adm⟩ := readapp_requires_adm sig doctrine forcedHighHigh A hReadApp
        exact adm.noDisqualifyingForcing trivial⟩⟩
...
theorem m1_skilled_harmonic_align_holds : HarmonicAlign sig doctrine skilled := by
  ...
  · intro z hz
    ...
    | .ext1 => trivial
    | .extBad => trivial
    | .extIrrelevant => exact hz.2.elim

theorem m1_bhc_good_layer1 :
    sig.HarmonicOutcome Consequence.harmoniousConsequence skilled :=
  bounded_harmonic_continuation sig doctrine skilled WorldModel.goodModel
    Consequence.harmoniousConsequence
    trivial
    m1_skilled_harmonic_align_holds
    trivial
    trivial
    rfl
    (fun _ _ _ _ _ => rfl)
...
theorem m1_bhc_bad_layer2_falsifier :
    ¬ (sig.ModelSufficient WorldModel.badModel skilled ∧ HarmonicAlign sig doctrine skilled ∧
  ...
  nonharmonic_falsifier sig doctrine skilled WorldModel.badModel Consequence.badConsequence
...
theorem N04_material_extension_misalignment_countermodel :
    MaterialExtensionOf sig Extension.extBad matExtMisaligned ∧
      ¬ AlignedExtension Extension.extBad matExtMisaligned :=
  ⟨⟨trivial, trivial⟩, fun h => h⟩

theorem m1_matExtMisaligned_harmonic_align_fails : ¬ HarmonicAlign sig doctrine matExtMisaligned := by
  intro h
  have := h.2.2.2.2.1 Extension.extBad N04_material_extension_misalignment_countermodel.1
  exact N04_material_extension_misalignment_countermodel.2 this

/-- 25h §15 SNS/Stuck threshold adversary. `tLow`: SNS evidence present,
pivot still available — SNS-like but not Stuck (Correction A: no
`StuckThresholdWitness` constructor can be built for `tLow`). -/
...
/-- `tStuck`: SNS evidence plus a genuine `StuckThresholdWitness`
(`staleForwardMotion`) — crosses the threshold into `Stuck`, which still
entails `SNSLike` (T13). -/
theorem T_adv15_tStuck : Stuck sig Trajectory.tStuck ∧ SNSLike sig Trajectory.tStuck :=
  ⟨⟨⟨Or.inl trivial, trivial⟩, StuckThresholdWitness.staleForwardMotion trivial⟩,
...
theorem T_adv15_tBusy : ¬ Stuck sig Trajectory.tBusy ∧ ¬ SNSLike sig Trajectory.tBusy := by
  have hns : ¬ SNSLike sig Trajectory.tBusy := by
    intro ⟨hha, _⟩
    rcases hha with h | h | h <;> exact h.elim
  exact ⟨fun h => hns (stuck_implies_sns sig Trajectory.tBusy h), hns⟩

/-- N06 `N06_sns_not_stuck_witness`: the user's obligation-list name for
the `tLow` countermodel above (SNS-like but not Stuck). -/
theorem N06_sns_not_stuck_witness : SNSLike sig Trajectory.tLow ∧ ¬ Stuck sig Trajectory.tLow :=
  T_adv15_tLow

/-- N07 `N07_high_activity_not_stuck_by_itself`: `skilled` carries high
`ActionEffort` yet `tBusy` (its trajectory-level analogue) is neither
`Stuck` nor `SNSLike` — high activity alone never crosses the threshold. -/
theorem N07_high_activity_not_stuck_by_itself :
    HighActionEffort 50 (actionEffort skilled) ∧
      ¬ Stuck sig Trajectory.tBusy ∧ ¬ SNSLike sig Trajectory.tBusy :=
  ⟨(by unfold HighActionEffort actionEffort; decide), T_adv15_tBusy.1, T_adv15_tBusy.2⟩
...
def ExtensionOf (_ : Extension) {K K' : Cut} (_ : Movement K K') : Prop := True
def ExtensionDiscriminates (_ : Extension) {K K' : Cut} (_ : Movement K K') : Prop := False
...
def OpportunityFieldUnavailable (_ : Trajectory) : Prop := False
def ForceResidueEvidence (_ : Trajectory) : Prop := False
