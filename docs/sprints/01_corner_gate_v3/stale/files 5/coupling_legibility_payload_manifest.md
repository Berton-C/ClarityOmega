...
```
3a2764428e43c9a96ec1a90066e442e0f9af8f03b450c46271f46c5742852347  coupling_legibility.metta        (unchanged)
50662dc45167fa044bec280d4da47f6f67dafaf8c10f7e6ce9c8a7a65ecfc940  coupling_legibility_writers.metta (A.4)
...
```
fc161738d475eb3546b200d5b51fd686b364bdd4d52374cc14a582cbd2b79a2a  apply_corner_gate_v3_monolith.py
31d4cba443eab64a72bf0edca7b7e56fe7b6b9251bd38c23d417dc3cbccef50f  validate_corner_gate_v3_monolith.py
```

## A.5 / B.3: D5 home amendment (Berton, 2026-07-10)

Ruling, verbatim intent: all v3 runtime MeTTa files live in soul/corner_gap/ with the existing v2 directory structure; helper functions belong in src/helper.py; consistency over distribution. This amends the D5 HOME (previously flat soul/); the D5 file NAMES keep the new-system naming per P9. Executed across the set: payload headers name their installed corner_gap path (pure and writers hashes refreshed, helper unchanged), apply destinations and import lines, validator installed paths and D1 order strings, reverse and expect-reversed checks, manifest target paths. Zero flat-soul references remain (grep-proven). Full mock cycle re-run green at the corner_gap home: dry zero FAILs with the corner_gap CREATE lines, apply green with both files landed beside the shim and backups, validator static 21 PASS 0 FAIL, reverse HASH-VERIFIED, expect-reversed 8 PASS, re-apply green. Archival scope unchanged and restated: corner_window_writers retired to .retired.v3, v2 gate body archived in its .bak, coupling_quantale_merge and state_delta files retained by design (live inputs), integrity-detector and continuity-probe files out of v3 scope pending a post-observation cleanup ruling.
Design doc canon amendment line 10: D5 home amended to soul/corner_gap/ (Berton 2026-07-10); naming unchanged.
