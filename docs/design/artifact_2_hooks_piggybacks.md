...
**Version:** v1.2 (May 1, 2026 afternoon; v1 April 30, v1.1 May 1 morning)
**Companion to:** loop.metta Wiring Diagram (Artifact 1), Growth Surface Map (Artifact 3), Triple Network Scaffold (Artifact 4), and ClarityOmega Cognitive Architecture Spec v3.0
**Purpose:** Answer "what will break if I touch this?" before making any change to loop.metta, helper.py, or active soul/ atoms. Trace the full transitive footprint of every hook and identify lines where one statement is doing several things.

**v1.2 changes:** Added constitutional-layer immutability note to soul/identity_kernel.metta entry per spec_v3.0 Section 0. Identity_kernel atoms are now identified as Layer 1+2 constitutional content with implications for the read-only partition work in Artifact 3 Sprint 11.
...
- 🧠 **Network:** SN (value-structure source). **Constitutional layer per spec_v3.0 Section 0:** atoms in identity_kernel are Layer 1 (the nine flourishings) and Layer 2 (priority hierarchy, tension vectors, irreversibility weights, paraconsistency pairs). Per the architectural commitment, these should live in a runtime-read-only AtomSpace partition. The mutation gate at line 126 is necessary but not sufficient protection. See Artifact 3 Sprint 11 (constitutional layer read-only partition).
...
- **Cleanup:** Atoms are queryable from anywhere - any new code that reads priority-rank or tension-vector depends on these specific atoms existing. **Once read-only partition is in place, write attempts to these atoms must fail at the partition level.**
