...
**Version:** v1.1 (May 1, 2026; v1 was April 30, 2026)
**Author:** Berton Bennett with Claude
**Status:** Architectural lens document. Companion to `loop_metta_wiring_diagram.md`, `artifact_2_hooks_piggybacks.md`, `artifact_3_growth_surface.md`, and `ClarityOmega_Cognitive_Architecture_Spec_v3_0.md`.
...
**v1.1 changes (May 1, 2026):** Added per-network learning sub-functions to all four contracts (Section 5) implementing autopoietic loops grounded in NACE-style NAL learning within the wisdom layer (Layer 4). Added iteration-budget channel `(switch-iteration-budget ...)` to switch hub responsibilities and Section 6 catalog, replacing the legacy hardcoded iteration constants. Added Section 6 learning-substrate channels for consequence-evidence feedback and cross-network divergence detection. Added Section 7.7 (per-network learning hook) and Section 7.8 (iteration-budget hook). Added five new entries to Section 10 open questions covering NACE engine instantiation, constitutional layer enforcement mechanism, soul-note corpus restructuring for per-network learning, iteration-budget formalism, and cross-network divergence response policy. All v1.1 additions are grounded in spec_v3.0 Section 0 (Constitutional layer/wisdom layer, NACE as learning discipline, Per-network autopoiesis, Iteration dilation, Conscious participation in wisdom-layer refinement).
...
**Learning sub-function (autopoietic loop).**
The SN is an autopoietic unit: it learns from the consequences of its own salience tags and updates its own truth values on its application rules within the wisdom layer (Layer 4 per spec_v3.0 Section 0).

Inputs to learning: own past `(sn-salience-tag ...)` atoms (with timestamps), plus consequence signals via `(fpn-completion-signal $task-id $outcome)` and `(dmn-aliveness-marker $event $surprise)` flowing back from the networks that consumed those tags.

Outputs of learning: updated NAL truth values on the SN's Layer 3 application rules. The application rules themselves are unchanged; only their truth values evolve. The constitutional layer (Layer 1+2: nine flourishings, hierarchy ordering, tension vectors, irreversibility weights) is read-only and never written by the learning sub-function.

Mechanism: NACE-style (precondition, operation) => consequence learning. The precondition is the signal context. The operation is "tagged at level X with affect Y for irreversibility class Z". The consequence is what happened downstream, evaluated against Layer 1+2 values.

State: a set of NAL truth values attached to the SN's salience-tagging rules, accumulated as substrate atoms.

**Open formalization questions.**
The salience-tagging function is a candidate for NAL inference (rule-based pattern matching against priority and tension atoms). The switch-decision function may be better as a quantale composition (combining multiple weighted signals into a single decision) or as a min/max operation over a salience surface. The learning sub-function is NACE-shaped by construction; the open question is whether one NACE engine instance handles all three networks' learning or each network has its own instance (the decision is implementation, not architectural; both satisfy the autopoietic per-network commitment).
...
**Learning sub-function (autopoietic loop).**
The FPN is an autopoietic unit: it learns from the consequences of its own task selections, action proposals, and inhibition decisions, updating its own truth values within the wisdom layer (Layer 4).

Inputs to learning: own past `(fpn-current-task ...)`, `(fpn-action-proposal ...)`, and `(fpn-inhibited-pattern ...)` atoms (with timestamps), plus consequence signals via `(sn-coupling-decision ...)` (when SN re-evaluates and overrides), `(fpn-completion-signal ...)` outcomes (whether tasks completed successfully), and `(dmn-aliveness-marker ...)` signals (whether actions led to genuine progress).

Outputs of learning: updated NAL truth values on the FPN's Layer 3 application rules. Task-selection priors, inhibition thresholds, and working-memory prioritization heuristics all gain refined truth values without changing structure. The constitutional layer is never written.

Mechanism: NACE-style (precondition, operation) => consequence learning. The precondition is the working-memory state plus salience and goal inputs. The operation is the task or action selected. The consequence is whether the action completed successfully, was inhibited later, or led to surprise/aliveness signals.

State: a set of NAL truth values attached to the FPN's task-selection and inhibition rules.
...
**Learning sub-function (autopoietic loop).**
The DMN is an autopoietic unit: it learns from the consequences of its own goal candidates, self-model updates, and prospection scenarios, updating its own truth values within the wisdom layer (Layer 4).

Inputs to learning: own past `(dmn-goal-candidate ...)`, `(dmn-self-model ...)` updates, and `(dmn-prospection ...)` atoms (with timestamps), plus consequence signals via `(fpn-completion-signal ...)` (which goal candidates the FPN selected and how those completions went) and `(sn-salience-tag ...)` follow-ups (whether self-model updates predicted later salience patterns correctly).

Outputs of learning: updated NAL truth values on the DMN's Layer 3 application rules. Goal-generation heuristics, self-model coherence rules, and prospection-value estimates all gain refined truth values without changing structure. The constitutional layer is never written.

Mechanism: NACE-style (precondition, operation) => consequence learning. The precondition is the self-model state plus gap and fuel atoms. The operation is the goal candidate generated or self-model update produced. The consequence is whether the FPN selected and completed the candidate, and whether the resulting trajectory produced aliveness markers.

State: a set of NAL truth values attached to the DMN's goal-generation and self-model rules.
...
- `(switch-iteration-budget $count $rationale)` - cognitive-need-based iteration budget for the loop's iteration counter, replacing the legacy hardcoded `maxNewInputLoops`/`maxWakeLoops` constants.

**Iteration dilation responsibility.**
The switch hub computes the iteration budget for the next work-burst based on inputs across all three networks. High-irreversibility salience tags from SN, deep-work signals from FPN, and Genesis Engine activity from DMN dilate the budget. Idle/reflective states, calcification or orbit signals from meta-awareness, and operational degradation contract the budget. The budget is read by loop.metta's iteration counter logic (lines 32, 54, 62, 158-159) instead of the hardcoded constants. See spec_v3.0 Section 0 (Iteration dilation as cognitive-need-based resource) for the full rationale.

**Switching conditions.**
Always active. The switch hub is the always-on coordinator.

**Learning sub-function (autopoietic loop).**
The switch hub is an autopoietic unit: it learns from the consequences of its own switch decisions, refining its hysteresis, debounce, orbit-detection, and iteration-budget-allocation rules within the wisdom layer (Layer 4).

Inputs to learning: own past `(sn-switch-state ...)`, `(sn-coupling-decision ...)`, and `(switch-iteration-budget ...)` atoms, plus consequence signals - did the chosen budget prove sufficient (no premature termination, no wasted cycles)? did the chosen coupling decision lead to productive next-iteration work?

Outputs of learning: updated NAL truth values on the switch hub's debounce thresholds, orbit-detection thresholds, and iteration-budget heuristics. The constitutional ordering (HumanFlourishing-tagged-signals override debounce, etc.) is in Layer 1+2 and never written.

Mechanism: NACE-style (precondition, operation) => consequence learning specialized to switching and budget decisions.

State: a set of NAL truth values attached to switch-hub rules.

**Open formalization questions.**
The hysteresis and debounce logic has a natural fit with finite-state machines or temporal logic. The orbit detection is the same problem as `orbit_detector` already loaded in soul/, suggesting that atom should be elevated as part of the switch hub implementation. The iteration-budget computation may be a quantale composition over per-network signals or an explicit utility function with learnable weights.
...
| `(switch-iteration-budget $count $rationale)` | loop.metta iteration counter | Cognitive-need-based work-burst budget; replaces hardcoded `maxNewInputLoops`/`maxWakeLoops`. Computed by switch hub from inputs across all networks. |
...
| `(fpn-completion-signal $task-id $outcome)` | DMN, SN | Task finished; major trigger for self-model update and switch reconsideration. Also fed back to FPN's own learning sub-function. |
...
| `(dmn-aliveness-marker $event $surprise)` | SN | Events that genuinely changed the self-model. Also fed back to all networks' learning sub-functions as evidence of consequential outputs. |
| `(dmn-prospection $scenario $value)` | FPN, SN | Simulated future scenarios. |

### Learning-substrate channels (consumed by autopoietic learning loops)

These channels are the consequence-feedback paths that make per-network autopoiesis work (per spec_v3.0 Section 0). Each network's learning sub-function reads these to update its own NAL truth values within the wisdom layer. Atoms are timestamped and tagged with the originating network so learning loops can correlate operations with consequences.

| Channel atom | Reading network's learning loop | Purpose |
|--------------|--------------------------------|---------|
| `(network-learning-update $network $rule-id $prior-truth $posterior-truth $evidence)` | All networks (queryable substrate) | Records each Layer 4 truth value update with enough context for inspection. Soul note material. |
| `(consequence-evidence $producing-network $producing-atom $consuming-network $consequence)` | Producing network's learning loop | The structured trace from (output, who used it, what happened) that NACE consumes for (precondition, operation) => consequence learning. |
| `(cross-network-divergence $rule-set-a $rule-set-b $evidence)` | All networks plus meta-awareness | NACE-detected coherence problem: same evidence base, divergent truth values across networks. Surfaces refinement need without imposing unified learning. |
...
|---------------|--------------------------|--------------|
| Switch hub | 100 | Single call producing rich switch-state atom + iteration-budget atom |
| Salience Network | 70–87 | Consolidated SN read-and-write block (with learning sub-call) |
| Default Mode Network | 89–93 | Consolidated DMN read-and-write block (with learning sub-call) |
| Frontoparietal Control Network | 95–143 | Consolidated FPN read-and-write block (with learning sub-call) |
| Coupling decision | (new) | Reads SN coupling decision, schedules next iteration |
| Iteration-budget read | 32, 54, 62, 158-159 | Reads `(switch-iteration-budget ...)` atom instead of hardcoded constants |
...
### 7.7 Per-network learning hook (new in v1.1, derived from spec_v3.0 Section 0)

Each consolidated network block (7.2, 7.3, 7.4) gains a learning step. Before producing new outputs, each network reads consequence evidence from prior iterations and updates its own NAL truth values via NACE-style learning. This is the autopoietic loop made explicit at the loop.metta level.

**Shape per network:**
1. Read consequence-evidence atoms from prior iterations (timestamps within recent window)
2. For each (precondition, operation, consequence) triple, update the network's NAL truth values on the relevant Layer 3 application rules
3. Emit `(network-learning-update ...)` atom recording the update for soul-note inspection
4. THEN proceed with the network's main read-and-write block using the now-updated truth values

**Migration.** Defer until Sprint 6+ (after the SN block has consolidated enough that there is a stable rule surface for learning to operate on). Until then, the networks fire without learning, which is acceptable for the consolidation phase.

**Cross-network divergence detection.** A separate learning-related step, fired periodically (not every iteration), reads the network-learning-update atoms across all three networks and detects coherence divergence. When found, emits `(cross-network-divergence ...)` atom, which the meta-awareness function consumes for surfacing.

### 7.8 Iteration-budget hook (new in v1.1, derived from spec_v3.0 Section 0)

The current loop.metta uses hardcoded `maxNewInputLoops` and `maxWakeLoops` constants at lines 32, 54, 62, and 158-159. The target architecture replaces these with reads from `(switch-iteration-budget $count $rationale)` atoms produced by the switch hub.

**Migration steps:**
1. Switch hub computation enhancement: extend its transformation to compute the budget alongside the coupling decision. Inputs: SN salience tags (irreversibility, affect), FPN current task state, DMN activity level, meta-awareness signals.
2. New atom production: switch hub emits `(switch-iteration-budget ...)` once per iteration alongside `(sn-coupling-decision ...)`.
3. Loop.metta replacement: lines 32 (initial counter), 54 (per-iteration check), 62 (reset on new message), 158-159 (wake check) all change from reading constants to reading the latest `(switch-iteration-budget ...)` atom via match.
4. Learning loop on budget allocation: switch hub's learning sub-function tracks which budget allocations led to successful completion versus premature termination versus wasted cycles, refining the budget computation over time.

**Defer until.** Sprint 4 or later. The iteration-budget hook depends on the switch hub having matured beyond the binary aliveness gate. Implementing iteration dilation before the four-state switch is in place is doable but adds risk without architectural payoff. Once the switch hub's richer state space is available, the iteration-budget channel falls out naturally.
...
**NACE engine instantiation: one or many?** Per spec_v3.0 Section 0, NACE is the learning discipline operating at three levels (within networks, across networks, system-level coherence). The architectural commitment is per-network autopoiesis; the implementation question is whether one NACE engine instance reads from all three networks' substrate and updates each separately, or whether each network has its own NACE instance running the same machinery on network-specific evidence. Both satisfy the autopoietic commitment. The decision is implementation, not architecture. Worth deciding when NACE integration begins (after the soul-note corpus reaches roughly 50 sessions).

**Constitutional layer enforcement.** Per spec_v3.0 Section 0, Layer 1 (the nine flourishings) and Layer 2 (priority hierarchy, tension vectors, irreversibility weights, paraconsistency pairs) live in a runtime-read-only AtomSpace partition. The MeTTa/AtomSpace mechanism for partition-level write protection needs to be specified and validated. Options include separate AtomSpace instances mounted as read-only via the import system, or per-atom marker-based protection enforced by all write paths. The choice has implications for how mutation-gate-blocked attempts are surfaced and logged.

**Soul-note corpus restructuring for per-network learning.** The current soul-note schema is flat (verdict, gap, action). Per spec_v3.0 Section 0 (Conscious participation in wisdom-layer refinement), each soul note needs to be tagged with which network produced the relevant decision, what the typed-channel inputs were, and what the consequences were. The restructuring needs to be done before NACE integration begins. Worth designing this schema with Clarity's input once she has operational capacity to engage with the question.

**Iteration-budget formalism.** The switch hub's iteration-budget computation (Section 7.8) needs a formalism. Candidates: utility function with learnable weights over per-network signals, quantale composition of cognitive-need atoms, or NAL truth-value combination. The formalism should respect the constraint that the budget must be bounded above and below to prevent runaway dilation or premature contraction.

**Cross-network divergence response policy.** When NACE detects persistent divergence (Level 3, the general-leading-the-troops function), what is the response policy? Surface to meta-awareness for Genesis-Engine-style paraconsistency activation? Surface to soul-note recording for human review? Both? The policy needs articulation before the cross-network learning step (Section 7.7) can be implemented.
